...
#ifdef FIREFLY_G20_F051
// Firefly hard-coded ESC settings for AM32 2.20 EEPROM layout (eeprom_version 3).
// Loaded into RAM at every boot via setFireflySettings(); flash is intentionally
// not written so the AM32 configurator may show stale values - that's expected.
// See doc/Firefly_Hardcoded_Settings.md for a per-byte breakdown.
const unsigned char fireflySettings[192] = {
    ...
    // bytes 24-35: pwm_freq=24(24kHz), startup_pwr=90, motor_kv=24(980KV), poles=14,
    //              brake_on_stop=0, stall=0, beep_vol=10, telem_interval=0,
    //              servo low=145(1040us), high=105(1960us), neutral=126(1500us), dead_band=50
    0x18, 0x5A, 0x18, 0x0E, 0x00, 0x00, 0x0A, 0x00, 0x91, 0x69, 0x7E, 0x32,
    // bytes 36-47: low_volt_cutoff=0(off), low_cell=50(3.0V/cell, moot), rc_car_rev=0,
    //              hall=0, sine_changeover=5, drag_brake=10, driving_brake=3(level 3),
    //              limits.temp=255(off), limits.current=0(off), sine_pwr=5,
    //              input_type=1(DSHOT), auto_advance=0(off)
    0x00, 0x32, 0x00, 0x00, 0x05, 0x0A, 0x03, 0xFF, 0x00, 0x05, 0x01, 0x00,
    // bytes 48-175: tune[128] - all 0xFF means use default Firefly 3-tone startup
    0x00, 0x78, 0x04, 0x04, 0x19, 0xBE, 0x7D, 0x00, 0x83, 0x3D, 0x00, 0x00,//60
    ...
    // bytes 176-191: can.* (16 bytes) - 0xFF means CAN unconfigured (this ESC does not use DroneCAN)
    0xFF, 0xFF, 0xFF, 0xFF,
...
{
    // Overwrite RAM eepromBuffer with hard-coded Firefly settings.
    // Flash is intentionally not updated (matches 2.189 behavior).
